...
{
    u8 objectEventId;
    TryGetObjectEventIdByLocalIdAndMap(LOCALID_FARAWAY_ISLAND_MEW, SAVEBLOCK_LOCATION.mapNum, SAVEBLOCK_LOCATION.mapGroup, &objectEventId);
...
void UpdateFarawayIslandStepCounter(void)
{
    ...
    if (SAVEBLOCK_LOCATION.mapNum == MAP_NUM(MAP_FARAWAY_ISLAND_INTERIOR)
     && SAVEBLOCK_LOCATION.mapGroup == MAP_GROUP(MAP_FARAWAY_ISLAND_INTERIOR))
    {
        steps++;
        if (steps >= 9999)
