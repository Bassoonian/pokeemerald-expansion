    ...
    else
    {
        ...
        else if (SetUpFieldMove(fieldMove) == TRUE)
        {
            ...
            case FIELD_MOVE_TELEPORT:
                mapHeader = Overworld_GetMapHeaderByGroupAndId(SAVEBLOCK_LAST_HEAL_LOCATION.mapGroup, SAVEBLOCK_LAST_HEAL_LOCATION.mapNum);
...
static bool8 GetBattleEntryEligibility(struct Pokemon *mon)
{
    ...
    if (GetMonData(mon, MON_DATA_IS_EGG)
        ...
        || (SAVEBLOCK_LOCATION.mapGroup == MAP_GROUP(MAP_BATTLE_FRONTIER_BATTLE_PYRAMID_LOBBY)
            && SAVEBLOCK_LOCATION.mapNum == MAP_NUM(MAP_BATTLE_FRONTIER_BATTLE_PYRAMID_LOBBY)
