...
static bool8 ShouldUseMultiCorridorDoor(void)
{
    ...
    {
        if (SAVEBLOCK_LOCATION.mapGroup == MAP_GROUP(MAP_BATTLE_FRONTIER_BATTLE_TOWER_MULTI_CORRIDOR)
            && SAVEBLOCK_LOCATION.mapNum == MAP_NUM(MAP_BATTLE_FRONTIER_BATTLE_TOWER_MULTI_CORRIDOR))
        {
            return TRUE;
        }
    }
